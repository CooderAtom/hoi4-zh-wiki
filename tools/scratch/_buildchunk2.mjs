// Build BULK chunk N for my range, reading page/slug/k VERBATIM from the pristine archived
// canonical file (tools/scratch/bulk/BULK.ALL.json) because data/pageblocks/BULK.ALL.json is gone.
//   node tools/scratch/_buildchunk2.mjs <N> <zhmaps.json...>
import fs from 'node:fs';
const N = Number(process.argv[2]);
const MAPS = process.argv.slice(3);
const dir = 'data/pageblocks';
const canon = 'tools/scratch/bulk/BULK.ALL.json';
const all = JSON.parse(fs.readFileSync(canon, 'utf8')).items.slice(0, 400);
...
console.log('canon=' + canon + '  wrote ' + file + '  items=' + out.length + '  chunkSize=' + chunk.length + '  problems=' + problems.length);
